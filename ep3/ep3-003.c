...
struct STpilha{
	int contextos; /* contexto: numeros de contextos que a celula possui  */
	contex iniContex,
	       finContex;
	pilha ant, pos;
	...
	do{
		...
		if(comando[0] == '.'){
			...
			switch(comando[0]){
				...
				break;
				case '-': 
					/* nesse programa também vamos supor que um texto só será ecluido quando a lista estiver vazia */
					comando++;
				break;
				case 'S':
					if(comando[1] != 0) menssagemErro();
