...
static pont head;
static pilha topoPilha;
static int N, lgN, tamPilha;
...
void STempilha(pont b){
	...
	tamPilha++;
	pilha aux = (struct STpilha *) malloc(sizeof(aux));
	...
	printf("%i %i %i, %i %i %i\n\n", aux -> tl[0][0], b -> ocor[0][0], topoPilha -> tl[0][0],aux -> tl[0][1], b -> ocor[0][1], topoPilha -> tl[0][1]);
}
...
int main (int argc, char* argv[])
{
	...
	do{
		...
		if(comando[0] == '.'){
			...
			switch(comando[0]){
				...
				case 'S':/*
					if(comando[1] != 0) menssagemErro();
						...
						topoPilha -> ant -> ant -> pos = topoPilha;
						topoPilha -> ant -> ant = topoPilha;
				...
				case 'N':
					if(comando[1] == 'P') /* Para saber quantos elementos estao na pilha */
						printf("\fThe stack has %i elements\n", tamPilha);
					else if(comando[1] == 'T'){
		...
		else if(comando[0] != 0){ /* programa deve calcular o conjunto S dos contextos em que <palavra> ocorre. */
			...
			if(aux){
				...
				printf("\tWord is not found, nothing was piled\n");
			}
		}
		/*imprime();*/
		if(topoPilha){
